#!/bin/bash
#SBATCH --job-name=metric_assess_sweep
#SBATCH --output=logs/metric_assess_sweep_%j.out
#SBATCH --error=logs/metric_assess_sweep_%j.err
...
if [[ -z "${SWEEP_ID}" ]]; then
  ...
  echo "Usage: sbatch scripts/sbatch_metric_assessment_sweep.sh SWEEP_ID [MAX_RUNS]"
  echo "Example full path: sbatch scripts/sbatch_metric_assessment_sweep.sh entity/project/sweep_id"
  exit 1
fi
...
echo "=========================================="
echo "Metric Assessment Sweep Agent"
echo "=========================================="
