...
class EllipseSequenceDataset(Dataset):
    ...
    def __init__(
        ...
        self.preloaded_data: Optional[torch.Tensor] = None

        # Optional path to pre-generated sequences: expected shape [N, T, 1, H, W] or [N, T, H, W]
        if data_path is not None:
            loaded = torch.load(data_path, map_location="cpu")
            if not isinstance(loaded, torch.Tensor):
                raise TypeError(f"Loaded data from {data_path} is not a torch.Tensor")
            self.preloaded_data = loaded
            self.num_sequences = int(loaded.shape[0])
            if loaded.dim() == 5:
                _, self.seq_len, _, self.H, self.W = loaded.shape
            elif loaded.dim() == 4:
                _, self.seq_len, self.H, self.W = loaded.shape
            else:
                raise ValueError(f"Unsupported tensor shape {loaded.shape} from {data_path}")
            return  # Skip procedural parameter generation when using cached data
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, int]:
        if self.preloaded_data is not None:
            seq = self.preloaded_data[idx]
            if seq.ndim == 3:  # (T, H, W) -> (T, 1, H, W)
                seq = seq.unsqueeze(1)
            return seq, 0
...
class DeterministicEllipseSequenceDataset(EllipseSequenceDataset):
    """
    Deterministic ellipse-sequence variant with ~2 degrees of freedom.

    Goal: make the full sequence uniquely determined by the first frame (t=0) so
    a model that encodes only x[:,0] can, in principle, reconstruct x[:,1:].

    Generative parameters (per sequence):
      - a0: major axis length (controls scale)
      - e0: eccentricity at t=0 (controls minor axis)

    Dynamics:
      - eccentricity evolves linearly with a deterministic delta that depends only on a0:
          e(t) = clip(e0 + alpha(t) * delta(a0), [min_e, max_e])

    Notes:
      - To keep the DoF at 2, you typically want: fix_center=True, fix_theta=True, fix_intensity=True.
      - This dataset pre-samples all parameters in __init__ (like the base class).
    """

    def __init__(
        self,
        *args,
        deterministic_delta_scale: float = 0.4,
        **kwargs,
    ) -> None:
        # Force a base 'linear' schedule in the parent (we overwrite the sampled params anyway).
        kwargs = dict(kwargs)
        kwargs["schedule_type"] = "linear"
        super().__init__(*args, **kwargs)

        # Rebuild parameters with deterministic two-DoF dynamics.
        rng = getattr(self, "_rng", None)
        if rng is None:
            rng = np.random.default_rng(42)
            self._rng = rng

        min_r = int(self.min_r)
        max_r = int(self.max_r)
        r_span = max(1, max_r - min_r)
        e_span = float(self.max_e - self.min_e)
        delta_scale = float(deterministic_delta_scale)

        # Defaults to reduce DoF (caller can override, but then DoF increases).
        if self.fix_center:
            global_cy = int(self.H // 2)
            global_cx = int(self.W // 2)
        if self.fix_theta:
        ...
        for _ in range(self.num_sequences):
            a0 = int(rng.integers(min_r, max_r + 1))
            e0 = float(rng.uniform(self.min_e, self.max_e))

            # Deterministic drift: continuous in a0, centered at 0, bounded by delta_scale * e_span.
            a_norm = float(a0 - min_r) / float(r_span)
            delta = (a_norm - 0.5) * (delta_scale * e_span)
            e_end = float(np.clip(e0 + delta, self.min_e, self.max_e))
            ...
            if self.fix_center:
                ...
                cy = float(self.H // 2 + rng.integers(-self.center_jitter, self.center_jitter + 1))
                cx = float(self.W // 2 + rng.integers(-self.center_jitter, self.center_jitter + 1))

            intensity = float(global_intensity if self.fix_intensity else rng.uniform(0.5, 1.0))

            params.append(
                {
                    "a0": a0,
                    "b0": a0,  # keep_major_axis_constant uses max(a0,b0)=a0, so b0 is irrelevant
                    "theta": theta,
                    "cy": cy,
                    "cx": cx,
                    "intensity": intensity,
                    "schedule": {"type": "linear", "start": e0, "end": e_end},
                    "cycle": False,
                }
            )
